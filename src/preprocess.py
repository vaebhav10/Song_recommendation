...
df.drop_duplicates(subset= ['artists', 'track_name','explicit'],inplace= True)
...
df['tags'] = df['artists'].str.replace(';', ' ') + ' ' +df['track_genre'] 
...
merged = np.concatenate([0.5* audio_data, tag_embeddings], axis = 1)
